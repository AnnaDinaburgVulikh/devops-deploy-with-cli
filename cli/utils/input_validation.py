...
def validate_port(value):
    ...
    try:
        ...
        raise ValueError(f"Invalid port '{value}'. Port must be a number between 1 and 65535.")
...
def validate_docker_image(value):
    ...
    if not re.match(docker_pattern, value):
        raise ValueError(f"Invalid Docker image format: '{value}'. Expected format: 'repository/name:tag'")
...
def validate_secret(ctx, param, value):
    ...
    if not re.match(vault_pattern, value):
        raise click.BadParameter("Secret name must be a valid HashiCorp Vault path (e.g., 'secret/data/my-secret').")
